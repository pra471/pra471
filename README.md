# 👋 Hi, I'm Y PrathapReddy (pra471)

<div align="center">

### 🤖 ML/DL Engineer | AI Researcher | Computer Vision Specialist

**Passionate about building intelligent systems and publishing cutting-edge research**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/yprathapredhyai)
[![Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/pra471)
[![Kaggle](https://img.shields.io/badge/Kaggle-20BEFF?style=for-the-badge&logo=kaggle&logoColor=white)](https://kaggle.com/pra471)
[![HuggingFace](https://img.shields.io/badge/🤗%20Hugging%20Face-FFD21E?style=for-the-badge)](https://huggingface.co/pra471)

</div>

---

## 🎓 About Me

- 🎓 **B.E. CSE-AI** Student | AI/ML Enthusiast
- 🔬 **Research Focus**: Deepfake Detection, Computer Vision, Generative AI
- 📚 **Research Paper Author** | Publishing in top-tier venues
- 🏆 **Active Competitor** in Kaggle competitions
- 💼 **Seeking**: AI/ML internships, EV Technology roles
- 📍 **Location**: India

---

## 💻 Tech Stack

<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![Keras](https://img.shields.io/badge/Keras-D00000?style=for-the-badge&logo=keras&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EEE?style=for-the-badge&logo=opencv&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black)

</div>

---

## 🚀 Featured Projects

### 🎯 Deepfake Detection System
- **Dual-stream CNN architecture** for real-time deepfake detection
- **98%+ accuracy** on benchmark datasets
- Advanced preprocessing & feature extraction techniques
- [Explore →](https://github.com/pra471/dual-stream_deepfake_detection)

### 🌸 Flower Species Classification
- Transfer learning with **EfficientNet** backbone
- **95%+ accuracy** on test dataset
- End-to-end ML pipeline with data augmentation
- [Explore →](https://github.com/pra471/Flower-Classification)

### 🏛️ 3D Bharath Cultural Tour
- Interactive **WebGL visualization** using Three.js
- Real-time 3D rendering of cultural landmarks
- Immersive user experience with detailed navigation
- [Explore →](https://github.com/pra471/3D-Bharath-Tour)

### 🛡️ Cybercrime Portal M1
- Full-stack web application for security incident reporting
- Real-time analytics dashboard
- Enterprise-level architecture
- [Explore →](https://github.com/pra471/CyberCrimePortalM1)

---

## 📊 GitHub Statistics

<div align="center">

![GitHub stats](https://github-readme-stats.vercel.app/api?username=pra471&show_icons=true&theme=tokyonight&count_private=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=pra471&layout=compact&theme=tokyonight)

</div>

---

## 🎯 Current Focus

- 📚 Publishing research papers in **IEEE** and **ACM** venues
- 🤖 Building production-level **AI/ML models**
- 💡 Exploring **Generative AI** applications
- 🏆 Competing in **Kaggle competitions**
- 🔧 Contributing to **open-source projects**

---

## 📫 Connect with Me

<div align="center">

💼 [LinkedIn](https://linkedin.com/in/yprathapredhyai) | 
🐦 [Twitter](https://twitter.com/pra471) | 
🎯 [Kaggle](https://kaggle.com/pra471) | 
🤗 [HuggingFace](https://huggingface.co/pra471)

</div>

---

<div align="center">

**Let's collaborate and build something amazing! 🚀**

*"Code is poetry written in a language computers understand."*

</div>
